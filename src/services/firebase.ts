...
export const createOboRelationship = async (
  userAccessToken: string,
  partnerBmId: string,
  clientBmId: string
) => {
  const response = await fetch(
    `https://graph.facebook.com/${FB_API_VERSION}/${partnerBmId}/managed_businesses?existing_client_business_id=${clientBmId}&access_token=${userAccessToken}`,
    { method: "POST" }
  );
...
export const createSystemUser = async (
  ...
  const response = await fetch(
    `https://graph.facebook.com/${FB_API_VERSION}/${clientBmId}/access_token?scope=ads_management,pages_read_engagement&app_id=${appId}&access_token=${partnerAdminToken}`,
    { method: "POST" }
  );
  return response.json();
};

export const getSystemUserId = async (systemUserToken: string) => {
  const response = await fetch(
    `https://graph.facebook.com/${FB_API_VERSION}/me?access_token=${systemUserToken}`
  );
  return response.json();
};
...
export const assignAssetToUser = async (
  ...
  const response = await fetch(
    `https://graph.facebook.com/${FB_API_VERSION}/${assetId}/assigned_users?user=${userId}&tasks=MANAGE&access_token=${userAccessToken}`,
    { method: "POST" }
  );
  return response.json();
};

export const getPageAccessToken = async (systemUserToken: string) => {
  const response = await fetch(
    `https://graph.facebook.com/${FB_API_VERSION}/me/accounts?access_token=${systemUserToken}`
  );
  return response.json();
};